...
TEST_F(OrderBookTest, CleanBidWithNonContiguousZeros) {
    // Zero out middle level (1517, vol=200)
    Order c1(OrderType::Cancel, Side::Bid, 1517, 200, 0);
    lob->process(c1);
    EXPECT_EQ(lob->best_bid(), 1519);
    EXPECT_EQ(lob->volume_at(Side::Bid, 1517), 0);

    // Zero out best (1519, vol=2000) — triggers clean
    Order c2(OrderType::Cancel, Side::Bid, 1519, 2000, 0);
    lob->process(c2);

    // New best should be 1518 (3000), middle zero at 1517 preserved, 1516 untouched
    EXPECT_EQ(lob->best_bid(), 1518);
    EXPECT_EQ(lob->best_bid_vol(), 3000);
    EXPECT_EQ(lob->volume_at(Side::Bid, 1517), 0);
    EXPECT_EQ(lob->volume_at(Side::Bid, 1516), 400);
}

TEST_F(OrderBookTest, CleanAskWithNonContiguousZeros) {
    // Zero out middle level (1522, vol=4400)
    Order c1(OrderType::Cancel, Side::Ask, 1522, 4400, 0);
    lob->process(c1);
    EXPECT_EQ(lob->best_ask(), 1520);
    EXPECT_EQ(lob->volume_at(Side::Ask, 1522), 0);

    // Zero out best (1520, vol=1200) — triggers clean
    Order c2(OrderType::Cancel, Side::Ask, 1520, 1200, 0);
    lob->process(c2);

    // New best should be 1521 (5100), middle zero at 1522 preserved, 1523 untouched
    EXPECT_EQ(lob->best_ask(), 1521);
    EXPECT_EQ(lob->best_ask_vol(), 5100);
    EXPECT_EQ(lob->volume_at(Side::Ask, 1522), 0);
    EXPECT_EQ(lob->volume_at(Side::Ask, 1523), 2300);
}

TEST_F(OrderBookTest, CleanCancelDeepThenBest) {
    Order c1(OrderType::Cancel, Side::Bid, 1516, 400, 0);
    lob->process(c1);
    Order c2(OrderType::Cancel, Side::Bid, 1517, 200, 0);
    lob->process(c2);
    Order c3(OrderType::Cancel, Side::Bid, 1518, 3000, 0);
    lob->process(c3);

    EXPECT_EQ(lob->best_bid(), 1519);
    EXPECT_EQ(lob->best_bid_vol(), 2000);

    Order c4(OrderType::Cancel, Side::Bid, 1519, 2000, 0);
    lob->process(c4);

    // All levels gone, regenerated at best_ask() - 5 = 1515
    EXPECT_EQ(lob->best_bid(), 1515);
    EXPECT_EQ(lob->volume_at(Side::Bid, 1514), 0);
    EXPECT_EQ(lob->volume_at(Side::Bid, 1513), 0);
    EXPECT_EQ(lob->volume_at(Side::Bid, 1512), 0);
}

TEST_F(OrderBookTest, CleanCancelDeepThenBestAsk) {
    Order c1(OrderType::Cancel, Side::Ask, 1523, 2300, 0);
    lob->process(c1);
    Order c2(OrderType::Cancel, Side::Ask, 1522, 4400, 0);
    lob->process(c2);
    Order c3(OrderType::Cancel, Side::Ask, 1521, 5100, 0);
    lob->process(c3);

    EXPECT_EQ(lob->best_ask(), 1520);
    EXPECT_EQ(lob->best_ask_vol(), 1200);

    Order c4(OrderType::Cancel, Side::Ask, 1520, 1200, 0);
    lob->process(c4);

    // All levels gone, regenerated at best_bid() + 5 = 1524
    EXPECT_EQ(lob->best_ask(), 1524);
    EXPECT_EQ(lob->volume_at(Side::Ask, 1525), 0);
    EXPECT_EQ(lob->volume_at(Side::Ask, 1526), 0);
    EXPECT_EQ(lob->volume_at(Side::Ask, 1527), 0);
}
...
TEST_F(OrderBookTest, CleanAfterSweepWithResidBid) {
    Order o(OrderType::Trade, Side::Bid, 1517, 5600, 0);
    lob->process(o);

    EXPECT_EQ(lob->best_bid(), 1516);
    EXPECT_EQ(lob->best_bid_vol(), 400);
    EXPECT_EQ(lob->best_ask(), 1517);
    EXPECT_EQ(lob->best_ask_vol(), 400);
    EXPECT_EQ(lob->volume_at(Side::Ask, 1518), 0);
    EXPECT_EQ(lob->volume_at(Side::Ask, 1519), 0);
    EXPECT_EQ(lob->volume_at(Side::Ask, 1520), 1200);
    EXPECT_THROW(lob->volume_at(Side::Ask, 1521), std::runtime_error);
    EXPECT_NO_THROW(lob->volume_at(Side::Bid, 1515));
    EXPECT_NO_THROW(lob->volume_at(Side::Bid, 1514));
    EXPECT_NO_THROW(lob->volume_at(Side::Bid, 1513));
    EXPECT_THROW(lob->volume_at(Side::Bid, 1512), std::runtime_error);
}

TEST_F(OrderBookTest, CleanAfterSweepWithResidAsk) {
    Order o(OrderType::Trade, Side::Ask, 1522, 11000, 0);
    lob->process(o);

    EXPECT_EQ(lob->best_ask(), 1523);
    EXPECT_EQ(lob->best_ask_vol(), 2300);
    EXPECT_EQ(lob->best_bid(), 1522);
    EXPECT_EQ(lob->best_bid_vol(), 300);
    EXPECT_EQ(lob->volume_at(Side::Bid, 1521), 0);
    EXPECT_EQ(lob->volume_at(Side::Bid, 1520), 0);
    EXPECT_EQ(lob->volume_at(Side::Bid, 1519), 2000);
    EXPECT_THROW(lob->volume_at(Side::Bid, 1518), std::runtime_error);
    EXPECT_NO_THROW(lob->volume_at(Side::Ask, 1524));
    EXPECT_NO_THROW(lob->volume_at(Side::Ask, 1525));
    EXPECT_NO_THROW(lob->volume_at(Side::Ask, 1526));
    EXPECT_THROW(lob->volume_at(Side::Ask, 1527), std::runtime_error);
}

TEST_F(OrderBookTest, CleanAfterAddInSpreadBid) {
    Order o(OrderType::Cancel, Side::Bid, 1519, 2000, 0);
    lob->process(o);
    o = Order(OrderType::Cancel, Side::Bid, 1518, 3000, 0);
    lob->process(o);
    o  = Order(OrderType::CreateAsk, Side::Ask, 1518, 657, 0);
    lob->process(o);

    EXPECT_EQ(lob->best_bid(), 1517);
    EXPECT_EQ(lob->best_bid_vol(), 200);
    EXPECT_EQ(lob->best_ask(), 1518);
    EXPECT_EQ(lob->best_ask_vol(), 657);
    EXPECT_EQ(lob->volume_at(Side::Ask, 1519), 0);
    EXPECT_EQ(lob->volume_at(Side::Ask, 1520), 1200);
    EXPECT_EQ(lob->volume_at(Side::Ask, 1521), 5100);
    EXPECT_THROW(lob->volume_at(Side::Ask, 1522), std::runtime_error);
}

TEST_F(OrderBookTest, CleanAfterAddInSpreadAsk) {
      Order o(OrderType::Cancel, Side::Ask, 1520, 1200, 0);
      lob->process(o);
      o = Order(OrderType::Cancel, Side::Ask, 1521, 5100, 0);
      lob->process(o);
      o = Order(OrderType::CreateBid, Side::Bid, 1521, 657, 0);
      lob->process(o);

      EXPECT_EQ(lob->best_ask(), 1522);
      EXPECT_EQ(lob->best_ask_vol(), 4400);
      EXPECT_EQ(lob->best_bid(), 1521);
      EXPECT_EQ(lob->best_bid_vol(), 657);
      EXPECT_EQ(lob->volume_at(Side::Bid, 1520), 0);
      EXPECT_EQ(lob->volume_at(Side::Bid, 1519), 2000);
      EXPECT_EQ(lob->volume_at(Side::Bid, 1518), 3000);
      EXPECT_THROW(lob->volume_at(Side::Bid, 1517), std::runtime_error);
  }
