...
void OrderBook::clean_bid() {
    if (bid_.empty()) {
        ...
        int32_t new_price = best_ask() - levels_ - 1;
        bid_[new_price] = std::max(1, sample_bid_level(0));
    }
    int32_t best = best_bid();
    
    while (!bid_.empty() && bid_.rbegin()->second == 0){
        bid_.erase(std::prev(bid_.end()));
    }
    
    if (static_cast<int>(bid_.size()) > levels_) {
        best = best_bid();
        for (int32_t i = 1; i < levels_; i++) {
            if (bid_.find(best-i) == bid_.end()) {
                bid_[best-i] = 0;
            }
        }
        while (static_cast<int>(bid_.size()) > levels_) {
            bid_.erase(bid_.begin());
        }
        return;
    }
    ...
    if (bid_.empty()) {
        ...
        int32_t new_price = best - levels_;
        bid_[new_price] = std::max(1, sample_bid_level(0));
    }
    ...
    for (int32_t i = 1; i < levels_; i++) {
        if (bid_.find(best-i) == bid_.end()) {
            bid_[best-i] = sample_bid_level(i);
        }
    }
    while (static_cast<int>(bid_.size()) > levels_) {
        bid_.erase(bid_.begin());
...
void OrderBook::clean_ask() {
    if (ask_.empty()) {
        ...
        int32_t new_price = best_bid() + levels_ + 1;
        ask_[new_price] = std::max(1, sample_ask_level(0));
    }
    int32_t best = best_ask();
    while (!ask_.empty() && ask_.begin()->second == 0) {
        ask_.erase(ask_.begin());
    }

    if (static_cast<int>(ask_.size()) > levels_) {
        best = best_ask();
        for (int32_t i = 1; i < levels_; i++) {
            if (ask_.find(best+i) == ask_.end()) {
                ask_[best+i] = 0; 
            }
        }
        while (static_cast<int>(ask_.size()) > levels_) {
            ask_.erase(std::prev(ask_.end()));
        }
        return;
    }
    ...
    for (int32_t i = 1; i < levels_; i++) {
        if (ask_.find(best+i) == ask_.end()) {
            ask_[best+i] = sample_ask_level(i);
        }
    }
