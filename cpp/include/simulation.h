...
namespace qr {
    ...
    // Strategy latency (DeltaDistrib, ~μs) races against market inter-event time (~ms).
    // Returns full event buffer; strategy trades also recorded in strat_buffer.
    ...
    // Lightweight HFT strategy simulation — stats only, no event buffer.
